...
func (c *Config) Validate() error {
	// In dry-run mode, set reasonable defaults for missing required fields
	if c.DryRun {
		if c.NodeName == "" {
			c.NodeName = "dry-run-node"
		}
		if c.KubernetesServiceHost == "" {
			c.KubernetesServiceHost = "kubernetes.default.svc.cluster.local"
		}
	} else {
		// In production mode, these fields are required
		if c.NodeName == "" {
